...
FROM
    `news`
WHERE   
    category='Sport' AND portal='Blic';
...
FROM
    `news`
WHERE   
   portal IN ('Blic','Kurir','Informer') AND category='Politika' ;
...
FROM
    `news`
WHERE   
   portal IN ('Blic','Kurir','Informer') AND category IN('Politika','Sport', 'Srbija') ;
...
FROM
    `news`
WHERE   
   on_homepage=1 AND (publish_time BETWEEN '2017-10-25 00:00:00' AND '2017-10-25 23:59:59');
...
SELECT
   category,
    COUNT(id) AS `broj_vesti`,
    SUM(facebook_likes) AS `ukupan_broj lajkova`
...
FROM
    `news`
GROUP BY category
ORDER BY SUM(facebook_likes) DESC,COUNT(id) DESC;
