...
if (isset($_POST["task"]) && $_POST["task"] == "save") {
    ...
    if (isset($_POST["price"]) && $_POST["price"] !== '') {
        ...
        if ($formData["price"] < 0) {
            $formErrors["price"][] = "Cena mora biti veca od nule";
        }
    ...
    if (isset($_POST["on_sale"]) && $_POST["on_sale"] !== '') {
        ...
        $on_SalePossibleValues = array("0", "1");

        //Validation videti da li je prosledjena vrednost medju opcijama
        if (!in_array($formData["on_sale"], $on_SalePossibleValues)) {
    ...
    if (isset($_POST["brand_id"]) && $_POST["brand_id"] !== '') {
        ...
        if (empty($testCategory)) {
            $formErrors["brand_id"][] = "Izabrali ste nepostojeci brend ";
        }
    ...
    if (empty($formErrors)) {
        ...
        if (isset($_FILES['photo'])) {
            ...
            $oldPhotoPart = __DIR__ . '/uploads/products/' . $product['photo_file_name'];
            if (is_file($oldPhotoPart)) {
                unlink($oldPhotoPart);
            }
